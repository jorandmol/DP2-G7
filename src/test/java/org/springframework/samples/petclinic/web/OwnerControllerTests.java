...
class OwnerControllerTests {
	...
	@WithMockUser(username = "admin1", password = "4dm1n", authorities = "admin")
	@Test
	void testShowOwnerWithoutPetsDisabled() throws Exception {
		mockMvc.perform(get("/owners/{ownerId}", TEST_OWNER_ID2)).andExpect(status().isOk())
				.andExpect(model().attribute("owner", hasProperty("lastName", is("Martin"))))
				.andExpect(model().attribute("owner", hasProperty("firstName", is("Angel"))))
				.andExpect(view().name("owners/ownerDetails"));
	}
